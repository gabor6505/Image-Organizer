import net.gabor6505.imageorganizer.Utils;
...
public class ImageLabel extends JLabel {
    ...
    private Icon loadImage(String filePath, int width, int height) {
        Image image = null;
    ...
    public void changeResolution(int width, int height) {
        Image image = rawImage.getScaledInstance(width, height, Image.SCALE_SMOOTH);

        if (image == null) return;
        setIcon(new ImageIcon(image));
    }

    public void setRawImage(BufferedImage raw) {
        rawImage = raw;
        Image image = rawImage.getScaledInstance(width, height, Image.SCALE_SMOOTH);
    ...
    /**
     * Overwrite to make label automatically resize on window resize
     */
    @Override
